...
describe("Sudoku Solver Utilities", () => {
  ...
  describe("updateCandidatesForBoxLineReduction", () => {
    test("should remove candidate from row outside block when limited to a row within block", () => {
      ...
      places.forEach((row) =>
        row.forEach((cell) => (cell.candidates = [5, 6])),
      );
  ...
  describe("X-Wing technique", () => {
    test("X-Wing (rows): should remove candidates when 2 rows share same 2 columns", () => {
      ...
      for (let col = 0; col < 9; col++) {
        if (col !== 2 && col !== 5) {
          places[0][col].candidates = places[0][col].candidates.filter(
            (c) => c !== 5,
          );
          places[3][col].candidates = places[3][col].candidates.filter(
            (c) => c !== 5,
          );
        }
      }
    ...
    test("X-Wing (columns): should remove candidates when 2 columns share same 2 rows", () => {
      ...
      for (let row = 0; row < 9; row++) {
        if (row !== 2 && row !== 6) {
          places[row][1].candidates = places[row][1].candidates.filter(
            (c) => c !== 7,
          );
          places[row][4].candidates = places[row][4].candidates.filter(
            (c) => c !== 7,
          );
        }
      }
      ...
      for (const row of [1, 4, 7]) {
        for (let col = 0; col < 9; col++) {
          if (col !== 3 && col !== 5 && col !== 8) {
            places[row][col].candidates = places[row][col].candidates.filter(
              (c) => c !== 4,
            );
          }
        }
  ...
  describe("Pointing Pair/Triple technique", () => {
    ...
    test("Pointing Pair (column): should remove candidates from outside block in same column", () => {
      ...
      for (let row = 3; row < 9; row++) {
        places[row][0].candidates = places[row][0].candidates.filter(
          (c) => c !== 7,
        );
      }
    ...
    test("Pointing Pair (single cell): should handle pointing single pattern", () => {
      ...
      for (let i = 0; i < 3; i++) {
        for (let j = 0; j < 3; j++) {
          if (i !== 0 || j !== 0) {
            places[i][j].candidates = places[i][j].candidates.filter(
              (c) => c !== 3,
            );
          }
        }
    ...
    test("should not modify when candidates are not in single row or column", () => {
      ...
      for (let col = 3; col < 9; col++) {
        ...
        places[1][col].candidates = places[1][col].candidates.filter(
          (c) => c !== 6,
        );
      }
      ...
      expect(JSON.stringify(places[0][3].candidates)).toBe(
        originalCandidate0_3,
      );
      expect(JSON.stringify(places[1][3].candidates)).toBe(
        originalCandidate1_3,
      );
    });
  });
});
