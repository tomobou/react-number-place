...
describe('NumberSelector Component', () => {
    ...
    test('should handle numeric string selectValue', () => {
        const mockClick = jest.fn();
        const { rerender } = render(<NumberSelector selectValue="1" onClick={mockClick} />);
    ...
    test('should only have one selected button at a time', () => {
        const mockClick = jest.fn();
        const { rerender } = render(<NumberSelector selectValue="3" onClick={mockClick} />);

        let buttons = screen.getAllByRole('button');
        let selectedCount = Array.from(buttons).filter(btn => btn.className.includes('selected')).length;
        expect(selectedCount).toBe(1);

        rerender(<NumberSelector selectValue="7" onClick={mockClick} />);
        buttons = screen.getAllByRole('button');
        selectedCount = Array.from(buttons).filter(btn => btn.className.includes('selected')).length;
        expect(selectedCount).toBe(1);
    });
    ...
    test('should handle all 10 values correctly', () => {
        ...
        values.forEach((value, index) => {
            const { unmount } = render(<NumberSelector selectValue={value} onClick={mockClick} />);
            const buttons = screen.getAllByRole('button');
