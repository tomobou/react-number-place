...
export class Game extends React.Component<GameProps, GameStates> {
  ...
  getPredictionTypeMessage(type: string) {
    switch (type) {
      case 'UNIQUE_PLACE':
        return '値確定';
      case 'OVERLAP_CONDITIONS':
        return '重複排除値確定';
      case 'UNIQUE_CANDIDATE':
        return '条件確定';
      case 'NAKED_RESERVATION':
        return '予約排除後条件確定';
      case 'X_WING':
        return 'X-Wing排除後条件確定';
      case 'SWORDFISH':
        return 'Swordfish排除後条件確定';
      default:
        return type;
    }
  }
  ...
  getSetValueHistoryPrediction(prediction: Prediction) {
    return this.getSetValueHistoryMessage(
      prediction.row,
      prediction.col,
      prediction.value.toString(),
      this.getPredictionTypeMessage(prediction.type),
    );
  }
  ...
  handleNextPrediction() {
    ...
    if (prediction != null) {
      ...
      let predictText =
        '[' + (prediction.row + 1) + ']行[' + (prediction.col + 1) + ']列目は' + prediction.value + 'です。';
    ...
    if (savedValues != null) {
      const squares = Array(9)
        ...
        .map(function (_, index) {
          return savedValues.slice(index * 9, (index + 1) * 9);
        });
      this.setState((state) => ({
        squares: squares,
      }));
  ...
  updateCandidatesList(row: number) {
    ...
    if (row > 5) {
      candidatesList = updateCandidatesForPlaceValue(createConditions(calcPlaces(this.state.squares))).map(
        function (places) {
      ...
      candidatesList = updateCandidatesForNakedReservations(
        updateCandidatesForOverlapConditions(
          updateCandidatesForPlaceValue(createConditions(calcPlaces(this.state.squares))),
        ),
      ).map(function (places) {
        return places.map(function (place) {
          return place.value != null ? place.value.toString() : place.candidates.join('');
      ...
      candidatesList = updateCandidatesForOverlapConditions(
        updateCandidatesForPlaceValue(createConditions(calcPlaces(this.state.squares))),
      ).map(function (places) {
        return places.map(function (place) {
          return place.value != null ? place.value.toString() : place.candidates.join('');
  ...
  render() {
    return (
      <div className="game">
        <Board squares={this.state.squares} onClick={(row, col) => this.handleClick(row, col)} />
        <NumberSelector selectValue={this.state.selectValue} onClick={(value) => this.handleSelect(value)} />
        <NextPrediction
          predictText={this.state.predictText}
          onClick={() => this.handleNextPrediction()}
        ></NextPrediction>
        ...
        <div className="candidates">
          ...
          <Board squares={this.state.candidatesList} onClick={(row, col) => this.updateCandidatesList(row)} />
        </div>
      </div>
    );
  }
