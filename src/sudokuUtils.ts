...
export function calcPlaces(squares: string[][]): Place[][] {
  return squares.map(function (row, rowIndex) {
    return row.map(function (cell, colIndex): Place {
      ...
      let candidates =
        '1' <= cell && cell <= '9' ? [] : Array.from({ length: 9 }, (_, index) => index + 1);
      return {
        value: value,
  ...
  conditions.forEach((condition) => {
    ...
    condition.forEach((place) => {
      place.candidates = place.candidates.filter(
        (candidate) => !definiteValues.some((defValue) => candidate === defValue)
      );
    });
  });
  return conditions;
      ...
      if (overlap.length > 0) {
        ...
        for (let overlapValue of [...overlapCandidates.values()]) {
          let overlapBlockLength = blockCondition.filter(
            (place) => !overlap.includes(place) && place.candidates.includes(overlapValue)
          ).length;
  ...
  for (let candidate = 1; candidate <= 9; candidate++) {
    ...
    for (let i = 0; i < n; i++) {
      for (let j = i + 1; j < n; j++) {
        ...
        const sameBlock =
          Math.floor(cell1.rowIndex / 3) === Math.floor(cell2.rowIndex / 3) &&
