    ...
    return (
      <div className="number-selector-row">
        {classNames.map((className, index) => {
          ...
          return (
            <button key={'number-selector-' + index} className={className} onClick={() => this.props.onClick(value)}>
              {value}
            </button>
          );
        })}
...
class HistoryView extends React.Component<HistoryViewProps> {
  render() {
    let history = this.props.history.map((message, index) => <div key={index}>{message}</div>);
