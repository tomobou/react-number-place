...
- nextを押すと、機械的に次の設定値の予測を行い値を設定します。
  ...
  - ポイントペア/トリプル（Pointing Pair/Triple）による候補除去
    - ブロック内の候補が2～3個のマスに限定されており、それらがすべて同じ行または列に位置している場合、その行・列の他のブロックに属するマスからその候補を除去します。
  - X-Wing テクニック
    - ある数字が正確に2つの行（または列）のみに候補として存在し、それぞれの行（列）で同じ2つの列（行）に位置している場合、その2つの列（行）の他の行（列）からその数字を除去します。
  - Swordfish テクニック
    - X-Wingを3行3列に拡張したテクニック。3つの行（列）で同じ数字が同じ3つの列（行）に候補として存在する場合、その3列（行）の他の行（列）からその数字を除去します。
- 操作履歴を表示します
